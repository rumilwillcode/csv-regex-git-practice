...
def create_call_list(customer_id, feedback, rating, phone_num):
  ...
  with open("unhappy_call_list.csv", "w", newline="") as file:
    ...
    for i in range(len(customer_id)):
      writer.writerow([customer_id[i], feedback[i], rating[i], phone_num[i]])

def main():
  """Executes unhappy_customer and create_call_list functions."""

  customer_id, feedback, rating, phone_num = unhappy_customer()
  create_call_list(customer_id, feedback, rating, phone_num)
